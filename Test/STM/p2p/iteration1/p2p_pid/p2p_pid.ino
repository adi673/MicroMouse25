...
void p2p_pid(MotorController& m1, MotorController& m2, int dist, double error_thresh = 3, int ramp_duration = 5000)  
{
    double setpnt_counts = m1.req_counts(dist);  // Ensure this function correctly converts distance to encoder counts
    Serial.print("Required counts: ");
    Serial.println(setpnt_counts);
    ...
    double kp_error = 0.4, kd_error = 1;
    double kp_sync = 0.08, kd_sync = 0.03;
    ...
    while (true) {
        ...
        double error_distance = setpnt_counts - count_pulses_left;
        int error_sync = count_pulses_left - count_pulses_right; 
        ...
        double derivative_error = (error_distance - previous_error);
        double derivative_sync = (error_sync - previous_sync);

        double pv_error = kp_error * error_distance + kd_error * derivative_error;
        double pv_sync = kp_sync * error_sync + kd_sync * derivative_sync;
        pv_sync = constrain(pv_sync, -15, 15);  
...
int left_speed = min((int)max(target_speed + pv_sync, initial_speed), 200);
int right_speed = min((int)max(target_speed - pv_sync, initial_speed), 200);

        if (setpnt_counts > 0) {  // Forward movement
            m1.rotate(1, left_speed);
            m2.rotate(1, right_speed);
